...
def create_ic(gridfile, icfile):
    ...
    iceConcentrationVar = ic.createVariable("iceAreaCell", 'd', ('nCells', 'ONE'))
    iceConcentrationVar[:,:] = iceConcentration[:,0,:]

    iceVolumeVar = ic.createVariable("iceVolumeCategory", 'd', ('nCells', 'nCategories', 'ONE'))
    iceVolumeVar[:,:,:] = iceVolume[:,:,:]

    iceVolumeVar = ic.createVariable("iceVolumeCell", 'd', ('nCells', 'ONE'))
    iceVolumeVar[:,:] = iceVolume[:,0,:]
