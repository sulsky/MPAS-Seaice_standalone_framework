...
def add_initial_area_volume_categories():
   ...
   # add decohesion initialization to new file
   posnMP = ds_new.variables["posnMP"][:]
   var1 = ds_new.createVariable("decohesionOpeningMP", "d", dimensions=["nParticles", "TWO"])
   var1[:]= 0.0
   var2 = ds_new.createVariable("decohesionNormalMP", "d", dimensions=["nParticles", "TWO"])
   var2[:]= 0.0
   for iParticle in range(0, nParticles):
     if (posnMP[iParticle,0] >= 80000 and posnMP[iParticle,0] <= 100000):
       var1[iParticle,0] = 0.8
       var2[iParticle,0] = 1.0
