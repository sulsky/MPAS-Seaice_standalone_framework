...
def advection_map(runtype):
    ...
    iceAreaCellSC  = file.variables["iceAreaCell"][:]

    file.close()

    # slotted cylinder
    iceAreaInitialSC = iceAreaCellSC[0,:]
    iceAreaSC = iceAreaCellSC[-1,:]

    # cosine bell
    filename = "./output_%s_%s_%s/output.2000.nc" %(experiment1,res,runtype)
    file = Dataset(filename, "r")
    iceAreaCellCB = file.variables["iceAreaCell"][:]
    file.close()

    iceAreaInitialCB = iceAreaCellCB[0,:]
    iceAreaCB        = iceAreaCellCB[-1,:]
